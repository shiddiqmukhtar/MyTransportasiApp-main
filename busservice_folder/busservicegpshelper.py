...
class BusServiceGpsHelper():
    has_centered_map = False
    dialog = None
    def run(self):
        # Get a reference to GpsBlinker, then call blink()
        bus_service_gps_blinker = App.get_running_app().busservicemapview.ids.busserviceblinker
 
        # Start blinking the GpsBlinker
        bus_service_gps_blinker.blink()
    ...
    def update_blinker_position(self, *args, **kwargs):
        ...
        bus_service_gps_blinker = App.get_running_app().busservicemapview.ids.busserviceblinker
        bus_service_gps_blinker.lat = my_lat
        bus_service_gps_blinker.lon = my_lon
 
        # Center map on gps
        if not self.has_centered_map:
            bus_service_map = App.get_running_app().busservicemapview
            bus_service_map.center_on(my_lat, my_lon)
