...
class BicycleParkingGpsHelper():
    has_centered_map = False
    dialog = None
    def run(self):
        # Get a reference to GpsBlinker, then call blink()
        bicycleparking_gps_blinker = App.get_running_app().bicycleparkingmapview.ids.bicycleparkingblinker
 
        # Start blinking the GpsBlinker
        bicycleparking_gps_blinker.blink()
    ...
    def update_blinker_position(self, *args, **kwargs):
        ...
        bicycleparking_gps_blinker = App.get_running_app().bicycleparkingmapview.ids.bicycleparkingblinker
        bicycleparking_gps_blinker.lat = my_lat
        bicycleparking_gps_blinker.lon = my_lon
 
        # Center map on gps
        if not self.has_centered_map:
            bicycleparking_map = App.get_running_app().bicycleparkingmapview
            bicycleparking_map.center_on(my_lat, my_lon)
