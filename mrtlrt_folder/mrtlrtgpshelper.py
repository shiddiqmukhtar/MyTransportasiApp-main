...
class MrtLrtGpsHelper():
    has_centered_map = False
    dialog = None
    def run(self):
        # Get a reference to GpsBlinker, then call blink()
        mrtlrt_gps_blinker = App.get_running_app().mrtlrtmapview.ids.mrtlrtblinker
 
        # Start blinking the GpsBlinker
        mrtlrt_gps_blinker.blink()
    ...
    def update_blinker_position(self, *args, **kwargs):
        ...
        mrtlrt_gps_blinker = App.get_running_app().mrtlrtmapview.ids.mrtlrtblinker
        mrtlrt_gps_blinker.lat = my_lat
        mrtlrt_gps_blinker.lon = my_lon
 
        # Center map on gps
        if not self.has_centered_map:
            mrtlrt_map = App.get_running_app().bicycleparkingmapview
            mrtlrt_map.center_on(my_lat, my_lon)
