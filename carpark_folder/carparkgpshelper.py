...
class CarParkGpsHelper():
    has_centered_map = False
    dialog = None
    def run(self):
        # Get a reference to GpsBlinker, then call blink()
        carpark_gps_blinker = App.get_running_app().carparkmapview.ids.carparkblinker 
 
        # Start blinking the GpsBlinker
        carpark_gps_blinker.blink()
    ...
    def update_blinker_position(self, *args, **kwargs):
        ...
        carpark_gps_blinker = App.get_running_app().carparkmapview.ids.carparkblinker
        carpark_gps_blinker.lat = my_lat
        carpark_gps_blinker.lon = my_lon
 
        # Center map on gps
        if not self.has_centered_map:
            carparkmap = App.get_running_app().carparkmapview
            carparkmap.center_on(my_lat, my_lon)
