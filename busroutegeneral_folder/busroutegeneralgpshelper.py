...
class BusRouteGeneralGpsHelper():
    has_centered_map = False
    dialog = None
    def run(self):
        # Get a reference to GpsBlinker, then call blink()
        bus_route_gps_blinker = App.get_running_app().busroutegeneralmapview.ids.busroutegeneralblinker
 
        # Start blinking the GpsBlinker
        bus_route_gps_blinker.blink()
    ...
    def update_blinker_position(self, *args, **kwargs):
        ...
        bus_route_gps_blinker = App.get_running_app().busroutegeneralmapview.ids.busroutegeneralblinker
        bus_route_gps_blinker.lat = my_lat
        bus_route_gps_blinker.lon = my_lon
 
        # Center map on gps
        if not self.has_centered_map:
            bus_route_map = App.get_running_app().busroutegeneralmapview
            bus_route_map.center_on(my_lat, my_lon)
