...
class IncidentsGpsHelper():
    has_centered_map = False
    dialog = None
    def run(self):
        # Get a reference to GpsBlinker, then call blink()
        incidents_gps_blinker = App.get_running_app().incidentsmapview.ids.incidentsblinker
 
        # Start blinking the GpsBlinker
        incidents_gps_blinker.blink()
    ...
    def update_blinker_position(self, *args, **kwargs):
        ...
        incidents_gps_blinker = App.get_running_app().incidentsmapview.ids.incidentsblinker
        incidents_gps_blinker.lat = my_lat
        incidents_gps_blinker.lon = my_lon
 
        # Center map on gps
        if not self.has_centered_map:
            incidents_map = App.get_running_app().incidentsmapview
            incidents_map.center_on(my_lat, my_lon)
