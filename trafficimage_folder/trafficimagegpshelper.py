...
class TrafficImageGpsHelper():
    has_centered_map = False
    dialog = None
    def run(self):
        # Get a reference to GpsBlinker, then call blink()
        traffic_image_blinker = App.get_running_app().trafficimagemapview.ids.trafficimageblinker
 
        # Start blinking the GpsBlinker
        traffic_image_blinker.blink()
    ...
    def update_blinker_position(self, *args, **kwargs):
        ...
        traffic_image_blinker = App.get_running_app().trafficimagemapview.ids.trafficimageblinker
        traffic_image_blinker.lat = my_lat
        traffic_image_blinker.lon = my_lon
 
        # Center map on gps
        if not self.has_centered_map:
            traffic_image_map = App.get_running_app().trafficimagemapview
            traffic_image_map.center_on(my_lat, my_lon)
